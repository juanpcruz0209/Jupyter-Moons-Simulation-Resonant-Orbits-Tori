...
# =============================================================================
# 1. SYSTEM CONFIGURATION & DATA SNAPSHOTS
# =============================================================================

# Global Configuration Holder (Will be updated by select_system)
ACTIVE_CONFIG = {
    'name': None,
    'mu': 0,
    'epsilon': 0,
    'L_dist': 1,    # Characteristic length (m)
    'R_jup_dim': 0, # Dimensionless Jupiter radius
    'R_moon_dim': 0 # Dimensionless Moon radius
}

# --- JUPITER DATA (Common Central Body) ---
DATA_JUPITER = {
...
# --- GANYMEDE SYSTEM DATA (Extracted from your previous string) ---
DATA_GANYMEDE = {
    ...
    'm': 1.48157e23,
    'epsilon': 0.0013, # Lower eccentricity for Ganymede
    'radius_km': 2634.1
}

# Strings for file export reference
...
def setup_system(choice):
    """Calculates MU, EPSILON and Collision Radii based on user choice."""
    global X_L4, Y_L4
    ...
    # 1. Calculate Distance (L) to normalize radii
    r_rel = target_data['r'] - DATA_JUPITER['r']
    dist_L = np.linalg.norm(r_rel)

    # 2. Calculate Mass Parameter (Mu)
    mu_val = target_data['m'] / (DATA_JUPITER['m'] + target_data['m'])

    # 3. Update Global Config
    ACTIVE_CONFIG['name'] = name
    ACTIVE_CONFIG['mu'] = mu_val
    ACTIVE_CONFIG['epsilon'] = target_data['epsilon']
    ACTIVE_CONFIG['L_dist'] = dist_L
    ACTIVE_CONFIG['target_body'] = target_data
    
    # 4. Calculate Dimensionless Collision Radii
    # R_dim = R_km * 1000 / L_meters
    ACTIVE_CONFIG['R_jup_dim'] = (DATA_JUPITER['radius_km'] * 1000) / dist_L
    ACTIVE_CONFIG['R_moon_dim'] = (target_data['radius_km'] * 1000) / dist_L

    # 5. Lagrange Points
    X_L4 = 0.5 - mu_val
    Y_L4 = np.sqrt(3) / 2

    print(f"\n✅ SYSTEM LOADED: JUPITER-{name}")
    print(f"   ├── Mu (Mass Param): {mu_val:.6e}")
    print(f"   ├── Epsilon (Ecc):   {ACTIVE_CONFIG['epsilon']}")
...
# =============================================================================
plt.rcParams.update({
    'figure.facecolor': '#000000', 'axes.facecolor': '#000000', 'savefig.facecolor': '#000000',
    'text.color': '#E0E0E0', 'axes.labelcolor': '#E0E0E0', 'xtick.color': '#E0E0E0', 'ytick.color': '#E0E0E0',
    'font.family': 'sans-serif', 'font.size': 10, 
    'axes.grid': True, 'grid.color': '#444444', 'grid.linestyle': ':', 'grid.alpha': 0.5,
    'lines.linewidth': 0.8, 'legend.facecolor': '#1A1A1A', 'legend.edgecolor': '#E0E0E0'
})

colors = {
    'jupiter': '#F4A460', 'secondary': '#A6D96A', # Generic green for Europa/Ganymede
    'spacecraft': '#FDFEFE', 'l4_marker': '#FF3333'
}

# =============================================================================
# 4. PHYSICS ENGINE (ER3BP)
# =============================================================================
...
class ProgressBar:
    ...
    def update(self, current):
        ...
        sys.stdout.flush()
    def finish(self): sys.stdout.write('\n')
...
def run_single_trajectory_vis():
    print(f"\nRunning Trajectory Visualization for {ACTIVE_CONFIG['name']}...")
    mu = ACTIVE_CONFIG['mu']
    eps = ACTIVE_CONFIG['epsilon']
    
    # Initial Conditions (Near L4)
    y_start = Y_L4 + 0.035 
    vx_start = 0.046939 
    ...
    # Bodies (Scaled manually for visibility in plot)
    ...
    ax2.set_title("3D Phase Space (X, Y, Vx)")
    ax2.set_zlabel("Vx")
    fig.colorbar(p, ax=ax2, label='Time')
    plt.show()

def run_poincare_map():
    print(f"\nRunning Poincaré Map for {ACTIVE_CONFIG['name']}...")
    mu = ACTIVE_CONFIG['mu']
    eps = ACTIVE_CONFIG['epsilon']
    ...
    ax.set_title(f"Poincaré Map ({ACTIVE_CONFIG['name']}): Y vs Vy")
...
# =============================================================================
# 6. DATA EXPORT MODULE (Generic for any Moon)
# =============================================================================
...
def run_file_generator():
    # Uses the currently active system configuration
    moon_data = ACTIVE_CONFIG['target_body']
    name = ACTIVE_CONFIG['name']
    
    print(f"\nGENERATING DATA FILE FOR: JUPITER-{name}")
    print("Transforming sample L4 orbits to Inertial Frame...")
    
    sys_params = analyze_snapshot(DATA_JUPITER, moon_data)
    
    # Simple grid of test candidates
    candidates = []
    candidates.append({'x': X_L4, 'y': Y_L4, 'vx': 0.0, 'vy': 0.0, 'label': 'L4_Center'})
    
    # Add some libration orbits
    for dy in [0.05, -0.05]:
    ...
    # Add reference to Io (always present in simulation background)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print("2. Visual Trajectory 2D/3D")
        print("3. GENERATE N-BODY FILE")
        print("0. Exit")
        
        choice = input("\nSelect Tool: ")
        if choice == '1': run_poincare_map()
        elif choice == '2': run_single_trajectory_vis()
        elif choice == '3': run_file_generator()
        elif choice == '0': break